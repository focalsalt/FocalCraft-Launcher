...
pub struct ProgressPayload {
    ...
    pub status_params: Option<std::collections::HashMap<String, String>>,
}
...
pub async fn import_pack(
    ...
    if pack_type == "modrinth" {
        ...
        let mut start_params = std::collections::HashMap::new();
        start_params.insert("total".to_string(), total_files.to_string());
        app.emit(
            "download-progress",
            ProgressPayload {
                instance_id: Some(instance_id.clone()),
                ...
                status_code: Some("status.modpack.import.start".to_string()),
                status_params: Some(start_params),
            },
        )
        .ok();
        ...
        for f in files_to_download {
            ...
            tasks.push(tokio::spawn(async move {
                ...
                *c += 1;
                ...
                let mut progress_params = std::collections::HashMap::new();
                progress_params.insert("current".to_string(), c.to_string());
                progress_params.insert("total".to_string(), total_files.to_string());
                app_h
                    .emit(
                        "download-progress",
                        ProgressPayload {
                            ...
                            status_code: Some("status.modpack.import.progress".to_string()),
                            status_params: Some(progress_params),
                        },
                    )
                    .ok();
            }));
...
pub async fn launch_instance(
    ...
    if is_fabric {
        ...
        let loader_version = match cfg.loader_version {
            Some(ref v) if !v.is_empty() => v.clone(),
            _ => {
                let loader_res = crate::get_client().clone()
                    .get(format!(
                        "https://meta.fabricmc.net/v2/versions/loader/{}",
                        query_version
                    ))
        ...
        if let Ok(res) = crate::get_client().clone().get(&profile_url).send().await {
